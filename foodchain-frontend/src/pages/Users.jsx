...
import { FiUserCheck, FiShield, FiMail, FiPhone } from 'react-icons/fi';
  ...
  if (user?.role !== 'ADMIN') {
    return (
      <div className="card text-center py-12">
        ...
        <p className="text-gray-600">Akses ditolak. Hanya Admin.</p>
      </div>
    );
  }

  if (loading) {
    return <div className="text-center py-12">Memuat pengguna...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">Kelola dan verifikasi pengguna di platform</p>
      </div>

      {/* Filter buttons */}
      <div className="flex flex-wrap gap-2">
        {[
          { key: 'all',        label: `Semua (${users.length})`,                          color: 'primary' },
          { key: 'verified',   label: `Terverifikasi (${users.filter(u=>u.isVerified).length})`,  color: 'green' },
          { key: 'unverified', label: `Pending (${users.filter(u=>!u.isVerified).length})`,       color: 'yellow' },
        ].map(f => (
        ...
        {['DONOR','STORE','RECIPIENT','COURIER'].map(role => (
          <button key={role} onClick={() => setFilter(role)}
            className={`px-3 py-1 rounded-full text-sm transition-colors ${
              filter === role ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Terdaftar</th>
              ...
              <tr key={u.walletAddress} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-mono">{u.walletAddress?.slice(0, 16)}...</p>
                    {u.email && (
                ...
                <td className="px-4 py-3">
                  <div className="space-y-1">
                    <span className={`inline-block px-2 py-1 text-xs rounded-full ${
                      ...
                      {u.isVerified ? '✓ Terverifikasi' : 'Pending'}
                    </span>
                    <span className={`inline-block ml-1 px-2 py-1 text-xs rounded-full ${
